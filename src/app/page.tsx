...
export default async function Home() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* ══ HERO SECTION ══ */}
      <section style={{
        background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 40%, #0c1526 100%)',
        ...
        {/* Stats Bar */}
        <div style={{
          maxWidth: '800px', margin: '60px auto 0',
          ...
          {stats.map((stat) => (
            <div key={stat.label} style={{ padding: '20px 16px', textAlign: 'center', background: 'rgba(15,23,42,0.6)' }}>
              <div style={{ fontSize: '1.6rem', fontWeight: '900', color: '#f59e0b', lineHeight: 1 }}>{stat.value}</div>
              <div style={{ fontSize: '0.72rem', color: '#64748b', marginTop: '4px', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px' }}>
            {[
              ...
              { icon: '🔗', title: 'Affiliate Disclosure', desc: 'We earn commissions but it never affects our scores.' },
            ].map((item) => (
